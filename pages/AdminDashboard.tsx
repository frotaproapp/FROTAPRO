...
export const AdminDashboard = () => {
  ...
  const handleEditTenant = async () => {
    if (!editingTenant || !editingTenant.name || !editingTenant.email) {
    ...
    setLoadingData(true);
    try {
      await api.admin.updateTenant(editingTenant);
      setShowEditModal(false);
      setEditingTenant(null);
      await loadTenants();
      alert("Tenant atualizado com sucesso!");
    } catch (e: any) { 
      console.error("❌ Erro detalhado na edição:", e);
      alert("Erro ao atualizar: " + (e.message || "Verifique o console para detalhes")); 
    } finally {
      setLoadingData(false);
    }
  };
    ...
    <div className="flex h-screen bg-slate-100 overflow-hidden font-sans">
                        ...
                        <td className="px-8 py-7 text-right">
                            <div className="flex justify-end gap-2">
                                <button 
                                    ...
                                    title="Editar Tenant"
                                    className="px-4 py-2 bg-blue-600 text-white rounded-xl text-[10px] font-black hover:bg-blue-700 transition-all uppercase tracking-tighter"
                                >
                                    EDITAR
                                </button>
                                <button 
      ...
      {showEditModal && editingTenant && (
        <div className="fixed inset-0 bg-slate-950/80 flex items-center justify-center z-[100] backdrop-blur-xl animate-fadeIn">
          <div className="bg-white p-10 rounded-[3rem] shadow-2xl w-[32rem] animate-slideUp max-h-[90vh] overflow-y-auto custom-scrollbar">
            <div className="flex justify-between items-start border-b border-slate-100 pb-6 mb-8">
              <h3 className="font-black text-3xl text-slate-900 tracking-tighter uppercase">Editar Tenant</h3>
              <button onClick={() => { setShowEditModal(false); setEditingTenant(null); }} className="p-2 hover:bg-slate-100 rounded-full transition-colors">
                <X size={24} className="text-slate-400" />
              </button>
            </div>
            ...
            <div className="space-y-6">
                <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block ml-1">Município / Órgão</label>
                    <input className="w-full border-2 border-slate-100 p-4 rounded-2xl font-bold outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 transition-all" placeholder="EX: PREFEITURA DE GOIÂNIA" value={editingTenant.name} onChange={(e) => setEditingTenant({...editingTenant, name: e.target.value.toUpperCase()})} />
                </div>
                ...
                <div>
                    ...
                    <textarea 
                      ...
                      onChange={(e) => setEditingTenant({...editingTenant, address: e.target.value.toUpperCase()})} 
                    />
                </div>

                <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block ml-1">E-mail do Administrador</label>
                    <input className="w-full border-2 border-slate-100 p-4 rounded-2xl outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 transition-all" placeholder="admin@municipio.gov.br" value={editingTenant.email} onChange={(e) => setEditingTenant({...editingTenant, email: e.target.value.toLowerCase()})} />
                </div>

                <div className="bg-slate-50 p-4 rounded-xl">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-1">Status do Tenant</label>
                    <div className="flex items-center gap-4">
                        <label className="flex items-center gap-2">
                            <input 
                                ...
                                onChange={() => setEditingTenant({...editingTenant, active: true})} 
                        ...
                        <label className="flex items-center gap-2">
                            <input 
                                ...
                                name="active" 
                                checked={!editingTenant.active} 
                                onChange={() => setEditingTenant({...editingTenant, active: false})} 
            ...
            <div className="flex justify-end gap-4 mt-10">
              <button onClick={() => { setShowEditModal(false); setEditingTenant(null); }} className="px-6 py-4 text-slate-400 font-black text-xs uppercase tracking-widest hover:text-slate-600 transition-colors">Cancelar</button>
              <button onClick={handleEditTenant} className="px-8 py-4 bg-blue-600 text-white rounded-2xl font-black shadow-2xl text-xs uppercase tracking-widest hover:bg-blue-700 transition-all flex items-center">
                {loadingData ? <RefreshCw className="animate-spin mr-2" size={16}/> : <Shield className="mr-2" size={16}/>}
                Atualizar Tenant
              </button>
            </div>
